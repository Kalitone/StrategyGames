...
public class peg5Cell {
    ...
    public boolean hasGreenPiece() {
        return (contents & 0b0111) != 0;
    }   
    ...
    public boolean canPlacePeg(int player) {
        if (player == peg5Board.PLAYER_0) {
            return (contents & (GREEN_PEG | GREEN_OPEN_TUBE)) == 0 && !hasPiece(player);
        } else {
            return (contents & (YELLOW_PEG | YELLOW_OPEN_TUBE)) == 0 && !hasPiece(player);
        }
    }

    public boolean canPlaceTube(int player) {
        if (player == peg5Board.PLAYER_0) {
            return (contents & (GREEN_OPEN_TUBE | GREEN_CLOSED_TUBE | YELLOW_CLOSED_TUBE)) == 0 && !hasPiece(player);
        } else {
            return (contents & (YELLOW_OPEN_TUBE | YELLOW_CLOSED_TUBE | GREEN_CLOSED_TUBE)) == 0 && !hasPiece(player);
        }
    }
